...
We're working hard to make *btxcore* the most powerful JavaScript library for working with bitcore. Our goal is to have *btxcore* be a library that can be used by anyone interested in bitcore, and to level expertise differences with great design and documentation.
...
These are some global design goals in btxcore that any change must adhere.
...
When possible, btxcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of btxcore methods. For example, calling `JSON.stringify` on an btxcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'btxcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the btxcore object.

DO:
```javascript
var btxcore = require('../');
var PublicKey = btxcore.PublicKey;
```
DON'T:
```javascript
